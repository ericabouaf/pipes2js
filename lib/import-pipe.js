...
var pipe2js = {
    ...
    importPipe: function (pipeId) {
        ...
        pipe2js.fetchPipe(pipeId, function (pipe) {
            ...
            runCode.push("if (process.argv.length < 3) {");
            runCode.push("  console.log('Please provide a pipe');");
            runCode.push("  process.exit(1);");
            runCode.push("}");
            runCode.push("var pipe = process.argv[2];");
            runCode.push("console.log('Running pipe: ' + pipe);");
            runCode.push("var deciderCode = fs.readFileSync(path.join(process.cwd(), 'pipes', pipe, pipe + '.js'), 'utf8');");
            runCode.push("var run = require('../lib/engine').run;");
