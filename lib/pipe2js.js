...
var pipe2js = {
    ...
    import: function (pipeId) {
        ...
        pipe2js.fetchPipe(pipeId, function (pipe) {
            ...
            fs.writeFileSync('pipes/' + pipeId + '/' + pipeId + '.js', deciderCode);
        });
        ...
        if (Array.isArray(conf)) {
            for (i = 0; i < conf.length; i += 1) {
                if (conf[i].hasOwnProperty("terminal")) {
                    pathsForTerminals[conf[i].terminal] = pathItems.concat(i);
                } else {
            ...
            pathsForTerminals[conf.terminal] = pathItems.concat([]);
        } else if (typeof conf === 'object') {
            for (k in conf) {
                if (conf.hasOwnProperty(k)) {
                    if (conf[k].hasOwnProperty("terminal")) {
                        pathsForTerminals[conf[k].terminal] = pathItems.concat(k);
                    } else {
                ...
                for (k in incomingValues) {
                    if (incomingValues.hasOwnProperty(k)) {
                        ...
                        if (!pathsForTerminals[k]) {
                            ...
                            if (pathsForTerminals[k].length > 0 && pathsForTerminals[k][0] === 'embed') {
                                pathsForTerminals[k] = ['embed'].concat(pathsForTerminals[k].slice(2));
                            }
                            pathToItem = pathsForTerminals[k].map(pathify).join('');
                        }

                        deciderCode.push("    params.input" + pathToItem + " = results(" + JSON.stringify(src.moduleid) + ")." + src.id + ";");
                    }
                }
